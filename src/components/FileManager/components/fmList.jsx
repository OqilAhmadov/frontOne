			...
			{({ items, isFetched, meta }) => {
				return (
					<div className="fm-list__wrapper">
						{activeFolder && <div className="ml-5 mr-5 fs-16 fw-700 mb-20">{activeFolder.title}</div>}
						...
						{filterType === "images" ? (
							<div className="fm-list">
								...
								{items.map(file => (
									<div
										className={`image-file ${get(selected, "id") === file.id ? "selected" : ""}`}
										key={file.id}
							...
							<div className="fm-list__doc">
								...
								{items.map(file => (
									<div
										className={`doc-file ${get(selected, "id") === file.id ? "selected" : ""}`}
										key={file.id}
										onClick={() => setSelected(file)}>
						...
						{isFetched && meta && meta.currentPage < meta.pageCount && <LoadMoreVisible setPage={() => setPage(meta.currentPage + 1)} />}
					</div>
				);
			}}
		</EntityContainer.All>
